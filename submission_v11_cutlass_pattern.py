...
True Warp Specialization using CUTLASS Pattern

VERSION: v11-cutlass-pattern
KEY CHANGES:
- Use pipeline.make_pipeline_state() for explicit state management
- Use ab_pipeline.producer_acquire(state) / consumer_wait(state) pattern
- 6 warps (192 threads): Warp 5=TMA, Warp 4=MMA, Warps 0-3=Epilogue
- Matches CUTLASS dense_blockscaled_gemm_persistent.py structure

Based on: https://github.com/NVIDIA/cutlass/blob/main/examples/python/CuTeDSL/blackwell/dense_blockscaled_gemm_persistent.py
"""
...
# 6-warp configuration matching CUTLASS example
EPILOG_WARP_IDS = (0, 1, 2, 3)  # Warps 0-3: Epilogue
MMA_WARP_ID = 4                 # Warp 4: MMA compute
TMA_WARP_ID = 5                 # Warp 5: TMA loads
threads_per_cta = 192           # 6 warps x 32 threads
...
def kernel_cutlass_pattern(
    ...
    """
    Warp-Specialized Kernel using CUTLASS Pattern

    Warp roles:
    - Warp 5 (TMA): Issues TMA loads, manages producer state
    - Warp 4 (MMA): Executes tensor core operations, manages consumer state
    - Warps 0-3 (Epilogue): Store results to global memory
    """
    ...
    # Create pipeline using CUTLASS pattern
    ...
    for tile_idx in range(bidz, total_tiles, num_sms):
        ...
        # =====================================================================
        # WARP-SPECIALIZED MAINLOOP (CUTLASS Pattern)
        # =====================================================================

        if warp_idx == TMA_WARP_ID:
            # -----------------------------------------------------------------
            # TMA WARP: Load matrices using producer pattern
            # -----------------------------------------------------------------
            ...
            # -----------------------------------------------------------------
            # MMA WARP: Compute using consumer pattern
            # -----------------------------------------------------------------
            ...
            for k_tile in range(k_tile_cnt):
                ...
                # Release slot for next iteration
...
def my_kernel_cutlass(
    ...
    """JIT wrapper for CUTLASS-pattern kernel."""
...
def compile_kernel(problem_sizes):
    ...
    cache_key = f"cutlass_{len(problem_sizes)}"
...
def custom_kernel(data: input_t) -> output_t:
    if len(data) == 4:
        ...
        raise ValueError("Expected 4-tuple group GEMM format")
