...
True 6-Warp Specialization with Persistent Scheduling

VERSION: v10-6warp
CHANGES:
- 192 threads (6 warps) like CUTLASS reference
- TMA warp (5): dedicated memory loads
- MMA warp (4): dedicated compute
- Epilogue warps (0-3): 4 parallel store warps
- Persistent kernel loop for tile processing

Based on CUTLASS grouped_blockscaled_gemm.py pattern that achieved high performance.
"""
...
# =============================================================================
# 6-WARP SPECIALIZATION CONFIGURATION (From CUTLASS Reference)
# =============================================================================

# Thread block: 6 warps x 32 threads = 192 threads
threads_per_cta = 192

# Warp roles - matching CUTLASS grouped_blockscaled_gemm.py
WARP_EPILOGUE_0 = 0   # Epilogue warp 0
WARP_EPILOGUE_1 = 1   # Epilogue warp 1
WARP_EPILOGUE_2 = 2   # Epilogue warp 2
WARP_EPILOGUE_3 = 3   # Epilogue warp 3
WARP_MMA = 4          # MMA compute warp
WARP_TMA = 5          # TMA load warp
...
# Persistent kernel: number of SMs
NUM_SMS = 132  # B200 has 208, using fewer for better occupancy


# =============================================================================
# 6-WARP PERSISTENT DEVICE KERNEL
# =============================================================================
...
def kernel_6warp(
    ...
    """
    6-Warp Specialized NVFP4 Group GEMM Kernel

    Warp roles (from CUTLASS):
    - Warp 5 (TMA): Issues TMA loads, runs ahead of compute
    - Warp 4 (MMA): Executes tensor core operations
    - Warps 0-3 (Epilogue): Parallel store operations
    """
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # Block index (SM identifier in persistent kernel)
    # -------------------------------------------------------------------------
    ...
    class SharedStorage:
        ...
        # Tile synchronization (between TMA and MMA warps)
        tile_ready: cutlass.Int32  # TMA signals tile data ready
        tile_done: cutlass.Int32   # MMA signals tile compute done
    ...
    # TMA warp is producer, MMA warp is consumer
    ...
    # -------------------------------------------------------------------------
    # Global Tensor Partitioning (shared by all warps)
    # -------------------------------------------------------------------------
    ...
    # TMEM allocation - all warps need access
    ...
    # =========================================================================
    # PERSISTENT TILE LOOP - Each CTA processes tiles: bidz, bidz+num_sms, ...
    # =========================================================================
    ...
    for tile_idx in range(bidz, total_tiles, num_sms):
        ...
        # Only warp 0 (or TMA warp) updates tensormaps
        ...
        # Slice TMA tensors for tile coordinates
        ...
        if warp_idx == WARP_TMA:
            # -----------------------------------------------------------------
            # TMA WARP (Warp 5): Issue TMA loads only
            # -----------------------------------------------------------------
            ...
            # -----------------------------------------------------------------
            # MMA WARP (Warp 4): Execute MMA compute only
            # -----------------------------------------------------------------
        ...
        # =====================================================================
        # EPILOGUE - All 6 warps participate (4 epilogue warps lead)
        # =====================================================================
...
# =============================================================================
# NON-PERSISTENT KERNEL (Fallback for small workloads)
# =============================================================================
...
def kernel(
    ...
    """Standard (non-persistent) kernel with 6-warp configuration."""
    warp_idx = cute.arch.warp_idx()
    warp_idx = cute.arch.make_warp_uniform(warp_idx)
    tidx, _, _ = cute.arch.thread_idx()
...
def my_kernel_6warp(
    ...
    """JIT wrapper for 6-warp persistent kernel."""
...
def my_kernel(
    ...
    """Standard JIT wrapper (non-persistent, 6-warp)."""
    tensor_of_abc_ptrs = cute.make_tensor(
        ptr_of_tensor_of_abc_ptrs, cute.make_layout((num_groups, 3), stride=(3, 1))
    )
    tensor_of_sfasfb_ptrs = cute.make_tensor(
...
def run_single_gemm(a, b, sfa_perm, sfb_perm, output, problem_sizes):
    """Execute a single block-scaled GEMM."""
