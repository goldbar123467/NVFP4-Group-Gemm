# NVFP4 Group GEMM - Reference Implementation

This is the **official reference implementation** that your kernel must match.
...
## Scale Factor Blocked Format Conversion
...
## Scale Factor Reordering (Pre-computation)

The kernel receives **pre-reordered** scale factors in this format:

```python
def create_reordered_scale_factor_tensor(l, mn, k, ref_f8_tensor):
    """
    Create the reordered scale factor tensor for the custom kernel.

    Output shape: (32, 4, rest_m, 4, rest_k, l)
    """
...
def _create_fp4_tensors(l, mn, k):
    """Generate FP4 tensors with valid bit patterns."""
...
def generate_input(m: tuple, n: tuple, k: tuple, g: int, seed: int):
    ...
    Returns:
        ...
        - sfasfb_tensors: list of (sfa, sfb) tuples (CPU, for reference)
        - sfasfb_reordered_tensors: list of (sfa_reordered, sfb_reordered) tuples (GPU)
