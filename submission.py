...
Persistent Warp-Specialized Implementation using CuTe DSL

VERSION: v12-persistent
CHANGES: Persistent tile loop (CTAs reuse across tiles),
         make_pipeline_state API with reset_count for correct inter-tile resets,
         conditional TMA descriptor update (skip when same group),
         true warp specialization (Warp 0=TMA, Warp 1=MMA),
         unpredicated epilogue for full tiles, fused SiLU gating
"""
...
NUM_SMS = 132  # B200 has 208 SMs; 132 balances occupancy
...
# Pipeline stages: 2-stage for warp specialization overlap
num_ab_stage = 2
num_acc_stage = 1
...
def kernel_persistent(
    ...
    - Persistent: CTAs loop over tiles (bidz, bidz+num_sms, ...)
    - True warp split: Warp 0=TMA producer, Warp 1=MMA consumer
    - Pipeline state API with reset_count for correct inter-tile phase
    - Conditional TMA update: skip descriptor rebuild for same group
    - Unpredicated epilogue for full (non-boundary) tiles
    """
    # -------------------------------------------------------------------------
    ...
    # =========================================================================
    # ONE-TIME SETUP (allocated once, reused across all tiles)
    # =========================================================================
    ...
    # --- Tensormap Manager & Init (once) ---
    tensormap_manager = utils.TensorMapManager(utils.TensorMapUpdateMode.SMEM, 128)
    # Index by bidz (SM index) — persistent kernel reuses per-SM workspace
    ...
    # Track group for conditional TMA update
    ...
    for tile_idx in range(bidz, total_tiles, num_sms):
        ...
        mC_mnl_iter = cute.make_ptr(
            c_dtype, tensor_of_abc_ptrs[group_idx, 2], cute.AddressSpace.gmem).align(32)
        mC_mnl_layout = cute.make_layout(
            (m, n, l), stride=(cute.assume(n, 32), 1, cute.assume(m * n, 32)))
        ...
        # --- Conditional TMA descriptor update (only when group changes) ---
        ...
        cute.arch.barrier()  # Sync: TMA update visible to all warps

        # --- K-tile count & TMA coordinate slicing ---
        ...
        # =================================================================
        # MAINLOOP — True Warp-Specialized 2-Stage Pipeline
        # =================================================================
        ...
        # Warps 2-3: skip mainloop, proceed to epilogue
        ...
        # Unpredicated fast-path for full tiles
...
# =============================================================================
# NON-PERSISTENT KERNEL (fallback for TASK format / small tile counts)
# =============================================================================
...
def _build_jit_common(problem_sizes, num_groups):
    """Build common JIT setup shared by persistent and non-persistent wrappers."""
    ...
    return (tiled_mma, tma_atom_a, tma_tensor_a, tma_atom_b, tma_tensor_b,
...
def my_kernel_persistent(
    ...
    # Tensormap workspace indexed by SM (num_sms entries, not total_tiles)
    tensor_of_tensormap = cute.make_tensor(
        ptr_of_tensor_of_tensormap, cute.make_layout((num_sms, 4, 16), stride=(64, 16, 1)))
...
def custom_kernel(data: input_t) -> output_t:
    if len(data) == 4:
        ...
        # Always use persistent kernel for GROUP GEMM evaluation
        # Tensormap workspace: NUM_SMS entries (not total_tiles)
        tensormap_shape = (NUM_SMS, num_tensormaps, bytes_per_tensormap // 8)
        tensor_of_tensormap = torch.empty(tensormap_shape, dtype=torch.int64, device="cuda")
        cute_ptr_of_tensor_of_tensormap = make_ptr(
            cutlass.Int64, tensor_of_tensormap.data_ptr(), cute.AddressSpace.gmem, assumed_align=16)
