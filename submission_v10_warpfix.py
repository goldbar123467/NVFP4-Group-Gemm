...
True Warp Specialization with 128 threads (4 warps)

VERSION: v10-warpfix
CHANGES:
- Keep 128 threads (4 warps) for MMA compatibility
- TRUE warp separation: Warp 0 = TMA, Warp 1 = MMA (separate loops)
- Persistent kernel pattern
- All 4 warps participate in epilogue

Key insight: Previous v9 had producer AND consumer in same warp (sequential).
This version has them in SEPARATE warps running CONCURRENTLY.
"""
...
# 128 threads = 4 warps (compatible with MMA)
threads_per_cta = 128

# Warp roles with true separation
WARP_TMA = 0      # Warp 0: TMA producer (issues loads)
WARP_MMA = 1      # Warp 1: MMA consumer (runs tensor cores)
# Warps 2-3: Available for epilogue (all 4 warps participate)
...
def kernel_warpspec(
    ...
    TRUE warp separation:
    - Warp 0: TMA producer loop (issues all k_tile loads)
    - Warp 1: MMA consumer loop (processes all k_tiles)
    ...
    sA = smem.allocate_tensor(element_type=ab_dtype, layout=a_smem_layout_staged.outer, byte_alignment=128, swizzle=a_smem_layout_staged.inner)
    sB = smem.allocate_tensor(element_type=ab_dtype, layout=b_smem_layout_staged.outer, byte_alignment=128, swizzle=b_smem_layout_staged.inner)
    sSFA = smem.allocate_tensor(element_type=sf_dtype, layout=sfa_smem_layout_staged, byte_alignment=128)
    sSFB = smem.allocate_tensor(element_type=sf_dtype, layout=sfb_smem_layout_staged, byte_alignment=128)

    # Pipeline: Warp 0 produces, Warp 1 consumes
    ...
    for tile_idx in range(bidz, total_tiles, num_sms):
        ...
        # Output tensor
        mC_mnl_iter = cute.make_ptr(c_dtype, tensor_of_abc_ptrs[group_idx, 2], cute.AddressSpace.gmem).align(32)
        ...
        mC_mnl_layout = cute.make_layout((m, n, l), stride=(cute.assume(n, 32), 1, cute.assume(m * n, 32),))
        mC_mnl = cute.make_tensor(mC_mnl_iter, mC_mnl_layout)
        gC_mnl = cute.local_tile(mC_mnl, cute.slice_(mma_tiler_mnk, (None, None, 0)), (coord_x, coord_y, 0))
        ...
        # Only TMA warp updates tensormaps
        if warp_idx == WARP_TMA:
        ...
        # =====================================================================
        # TRUE WARP SPECIALIZATION: Separate loops for TMA and MMA
        # =====================================================================

        if warp_idx == WARP_TMA:
            # -----------------------------------------------------------------
            # TMA WARP: Issues ALL TMA loads, then exits
            # Runs AHEAD of MMA warp due to async nature
            # -----------------------------------------------------------------
        ...
        elif warp_idx == WARP_MMA:
            # -----------------------------------------------------------------
            # MMA WARP: Waits for data, computes ALL k_tiles
            # Runs BEHIND TMA warp, consuming data as it arrives
            # -----------------------------------------------------------------
...
# =============================================================================
# NON-PERSISTENT KERNEL (fallback)
# =============================================================================
...
def kernel(
    ...
    """Non-persistent kernel with true warp separation."""
    warp_idx = cute.arch.warp_idx()
    warp_idx = cute.arch.make_warp_uniform(warp_idx)
    tidx, _, _ = cute.arch.thread_idx()
...
def my_kernel_warpspec(
    ...
    """JIT wrapper for warp-specialized persistent kernel."""
    ...
    tensor_of_tensormap = cute.make_tensor(ptr_of_tensor_of_tensormap, cute.make_layout((total_tiles, 4, 16), stride=(64, 16, 1)))
    ...
    initial_a = cute.make_tensor(
        ...
        cute.make_layout((min_a_shape[0], cute.assume(min_a_shape[2], 32), min_a_shape[3]),
                         stride=(cute.assume(min_a_shape[2], 32), 1, cute.assume(min_a_shape[0] * min_a_shape[2], 32))))
    initial_b = cute.make_tensor(
        cute.make_ptr(ab_dtype, 0, cute.AddressSpace.gmem, assumed_align=16),
        cute.make_layout((min_b_shape[1], cute.assume(min_b_shape[2], 32), min_b_shape[3]),
    ...
    mma_op = tcgen05.MmaMXF4NVF4Op(sf_dtype, (mma_tiler_mnk[0], mma_tiler_mnk[1], mma_inst_shape_k), tcgen05.CtaGroup.ONE, tcgen05.OperandSource.SMEM)
    ...
    tma_atom_a, tma_tensor_a = cute.nvgpu.make_tiled_tma_atom_A(cpasync.CopyBulkTensorTileG2SOp(tcgen05.CtaGroup.ONE), initial_a, a_smem_layout, mma_tiler_mnk, tiled_mma, cluster_layout_vmnk.shape)
    b_smem_layout = cute.slice_(b_smem_layout_staged, (None, None, None, 0))
    tma_atom_b, tma_tensor_b = cute.nvgpu.make_tiled_tma_atom_B(cpasync.CopyBulkTensorTileG2SOp(tcgen05.CtaGroup.ONE), initial_b, b_smem_layout, mma_tiler_mnk, tiled_mma, cluster_layout_vmnk.shape)
    sfa_smem_layout = cute.slice_(sfa_smem_layout_staged, (None, None, None, 0))
    tma_atom_sfa, tma_tensor_sfa = cute.nvgpu.make_tiled_tma_atom_A(cpasync.CopyBulkTensorTileG2SOp(tcgen05.CtaGroup.ONE), initial_sfa, sfa_smem_layout, mma_tiler_mnk, tiled_mma, cluster_layout_vmnk.shape, internal_type=cutlass.Int16)
    sfb_smem_layout = cute.slice_(sfb_smem_layout_staged, (None, None, None, 0))
    tma_atom_sfb, tma_tensor_sfb = cute.nvgpu.make_tiled_tma_atom_B(cpasync.CopyBulkTensorTileG2SOp(tcgen05.CtaGroup.ONE), initial_sfb, sfb_smem_layout, mma_tiler_mnk, tiled_mma, cluster_layout_vmnk.shape, internal_type=cutlass.Int16)
...
def my_kernel(
    ...
    """Standard JIT wrapper (non-persistent)."""
    tensor_of_abc_ptrs = cute.make_tensor(ptr_of_tensor_of_abc_ptrs, cute.make_layout((num_groups, 3), stride=(3, 1)))
    tensor_of_sfasfb_ptrs = cute.make_tensor(ptr_of_tensor_of_sfasfb_ptrs, cute.make_layout((num_groups, 2), stride=(2, 1)))
    tensor_of_problem_sizes = cute.make_tensor(ptr_of_tensor_of_problem_sizes, cute.make_layout((num_groups, 4), stride=(4, 1)))
    tensor_of_tensormap = cute.make_tensor(ptr_of_tensor_of_tensormap, cute.make_layout((total_num_clusters, 4, 16), stride=(64, 16, 1)))
...
def compile_kernel(problem_sizes):
    ...
    compiled_func = cute.compile(
        my_kernel,
        cute_ptr_of_tensor_of_problem_sizes, cute_ptr_of_tensor_of_abc_ptrs, cute_ptr_of_tensor_of_sfasfb_ptrs,
        cute_ptr_of_tensor_of_tensormap, total_num_clusters, problem_sizes, num_groups)
...
def compile_persistent_kernel(problem_sizes):
    ...
    compiled_func = cute.compile(
        my_kernel_warpspec,
        cute_ptr_of_tensor_of_problem_sizes, cute_ptr_of_tensor_of_abc_ptrs, cute_ptr_of_tensor_of_sfasfb_ptrs,
        cute_ptr_of_tensor_of_tensormap, total_tiles, num_sms, problem_sizes, num_groups)
...
def run_single_gemm(a, b, sfa_perm, sfb_perm, output, problem_sizes):
    ...
    cute_ptr_of_tensor_of_abc_ptrs = make_ptr(cutlass.Int64, tensor_of_abc_ptrs.data_ptr(), cute.AddressSpace.gmem, assumed_align=16)
    cute_ptr_of_tensor_of_sfasfb_ptrs = make_ptr(cutlass.Int64, tensor_of_sfasfb_ptrs.data_ptr(), cute.AddressSpace.gmem, assumed_align=16)
    cute_ptr_of_tensor_of_problem_sizes = make_ptr(cutlass.Int32, tensor_of_problem_sizes.data_ptr(), cute.AddressSpace.gmem, assumed_align=16)
    cute_ptr_of_tensor_of_tensormap = make_ptr(cutlass.Int64, tensor_of_tensormap.data_ptr(), cute.AddressSpace.gmem, assumed_align=16)

    compiled_func(cute_ptr_of_tensor_of_problem_sizes, cute_ptr_of_tensor_of_abc_ptrs, cute_ptr_of_tensor_of_sfasfb_ptrs,
                  cute_ptr_of_tensor_of_tensormap, total_num_clusters, problem_sizes, num_groups)
...
def custom_kernel(data: input_t) -> output_t:
    if len(data) == 4:
        ...
        if USE_PERSISTENT_KERNEL and total_tiles > NUM_SMS:
            ...
            compiled_func = compile_kernel(problem_sizes)
            compiled_func(cute_ptr_of_tensor_of_problem_sizes, cute_ptr_of_tensor_of_abc_ptrs, cute_ptr_of_tensor_of_sfasfb_ptrs,
                          cute_ptr_of_tensor_of_tensormap, total_tiles, problem_sizes, num_groups)
